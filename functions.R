...
chisq_to_csv <- function(chisq_obj, name = "chisq", direc = "./output/") {
  if (!inherits(chisq_obj, "htest")) {
    stop("Input must be an 'htest' object generated by chisq.test().")
  }
  
  # Extract key metrics into a tibble
  results_tbl <- tibble::tibble(
    statistic = as.numeric(chisq_obj$statistic),
    df        = as.numeric(chisq_obj$parameter),
    p_value   = chisq_obj$p.value,
    method    = chisq_obj$method
  )
  
  # Export to CSV
  write_csv(results_tbl, paste0(direc,name,".csv"))
